...
public class UsersQueriesHandler {
	...
	public User findUser(FetchUserPaymentDetailsQuery query) {
		...
		if (userOptional.isPresent()) {
			...
			User user = User.builder().userId(userDetailEntity.getUserId()).username(userDetailEntity.getUsername())
	...
	public List<UserRestModel> findUser(FindUsersQuery findUsersQuery) {
		List<UserDetailEntity> allEntity = userDetailsRepository.findAll();
		List<UserRestModel> users = new ArrayList<>();

		for (UserDetailEntity entity : allEntity) {
			...
			BeanUtils.copyProperties(entity.getPaymentDetail(),paymentModel);
			
			RoleRestModel roleRestModel = new RoleRestModel();
			BeanUtils.copyProperties(entity.getRole(),roleRestModel);
			
			userModel.setPaymentDetail(paymentModel);
			userModel.setRole(roleRestModel);
